...
static list<string> parse_expr(string expr) {
    ...
    while (expr.length() != 0) {
        ...
        // check if expression is just a boolean, is a variable in program state, or is a string TODO: this logic is lazy and wrong
            ...
            //TODO: this is sometimes an infinite loop. 
            index++;
        }
    }
    return tokens;
...
// executes all lines, is recursive
static int execute(list<Line> lines, int currentLevel) {
    ...
    while (it != lines.end()) {
        ...
        // assignment
        else if (contents.length() >= 4 && contents.rfind("let ") == 0) {
